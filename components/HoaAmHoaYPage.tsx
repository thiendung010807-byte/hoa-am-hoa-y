...
import { CalendarDays, Clock3, MapPin, Shirt, Backpack, UsersRound, ArrowDown, ArrowRight, Volume2, VolumeX, Sparkles, Heart, Star, Camera, Flame, PartyPopper, Music2, AudioLines, Headphones, Zap } from "lucide-react";
    ...
    {opened && <main>
      ...
      <section className="hero section">
        ...
        <motion.div className="hero-copy" initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{duration:.8}}>
          ...
          <div className="hero-actions"><a className="btn primary" href="#story">KHÁM PHÁ <ArrowDown size={18}/></a><a className="btn ghost" href="/dang-ky">ĐĂNG KÝ THAM GIA</a></div>
        </motion.div>
      ...
      <section className="story section" id="story"><div className="section-number">01</div><span className="eyebrow">HÒA ÂM HỎA Ý LÀ GÌ?</span><h2>Khác thanh âm.<br/><i>Chung một ngọn lửa.</i></h2><p>{event.intro}</p><div className="concept-duo"><motion.article whileHover={{y:-7,rotate:-1}} className="concept-card harmony"><div className="concept-icon"><AudioLines/></div><small>01 / HÒA ÂM</small><h3>Nghe nhau. Bắt nhịp nhau.</h3><p>Mỗi người là một thanh âm khác biệt. Khi gặp đúng nhau, những cá tính riêng tạo thành một giai điệu chung.</p><div className="mini-eq">{Array.from({length:12},(_,i)=><i key={i}/>)}</div></motion.article><motion.article whileHover={{y:-7,rotate:1}} className="concept-card fire"><div className="concept-icon"><Flame fill="currentColor"/></div><small>02 / HỎA Ý</small><h3>Ý tưởng chạm nhau. Lửa bật lên.</h3><p>Không chỉ gặp mặt, đây là lúc năng lượng, nhiệt huyết và những ý tưởng trẻ được truyền từ người này sang người khác.</p><div className="flame-row"><Flame/><Flame/><Flame/></div></motion.article></div><div className="value-grid">{[["🎧","Bắt nhịp"],["🔥","Truyền lửa"],["✨","Bật ý tưởng"],["🤝","Kết nối"]].map(([icon,text],i)=><motion.div whileHover={{y:-6,rotate:i%2?1:-1}} className="value-card" key={text}><b>{icon}</b><span>{text}</span></motion.div>)}</div></section>
      ...
      <section className="count-section section"><span className="eyebrow light">CHÚNG TA SẼ GẶP NHAU SAU</span><Countdown/><div className="calendar-info"><MiniCalendar/><div className="event-card"><span className="tape">HÒA ÂM HỎA Ý PASS</span><div><CalendarDays/><p><small>NGÀY</small><b>{event.dateLabel}</b></p></div><div><Clock3/><p><small>THỜI GIAN</small><b>{event.timeLabel}</b></p></div><div><MapPin/><p><small>ĐỊA ĐIỂM</small><b>{event.location}</b></p></div><div><Shirt/><p><small>DRESSCODE</small><b>{event.dressCode.join(" • ")}</b></p></div><div><Backpack/><p><small>MANG THEO</small><b>{event.bring}</b></p></div><div><UsersRound/><p><small>ĐỐI TƯỢNG</small><b>{event.audience}</b></p></div></div></div></section>

      <section className="dress section"><div><span className="eyebrow">WEAR YOUR VIBE</span><h2>DRESS<br/><i>CODE</i></h2><p>Mặc gì cũng được, miễn là bạn thấy tự tin. Nhưng nếu muốn cả hội lên ảnh thật xinh thì hãy ưu tiên…</p></div><div className="dress-note"><span>BLUE</span><span>WHITE</span><span>JEANS</span><div className="swatches"><i/><i/><i/></div><b>See you looking awesome! ✦</b></div></section>
      ...
      <section className="memories section"><span className="eyebrow">HÒA ÂM HỎA Ý SẼ CÓ…</span><h2>Những thứ đáng để<br/><i>nhớ thật lâu.</i></h2><div className="memory-grid">{[["01","TRÒ CHƠI"],["02","LÀM QUEN"],["03","CHỤP ẢNH"],["04","HOẠT ĐỘNG NHÓM"],["05","GIAO LƯU"],["06","CƯỜI THẬT NHIỀU"]].map(([n,t],i)=><motion.div className={`memory m${i+1}`} whileHover={{scale:1.03,rotate:i%2?1.5:-1.5}} key={n}><span>{n}</span><Camera/><b>{t}</b></motion.div>)}</div></section>
